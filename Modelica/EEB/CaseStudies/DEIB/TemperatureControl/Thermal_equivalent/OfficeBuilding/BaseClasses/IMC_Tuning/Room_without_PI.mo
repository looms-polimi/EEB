...
model Room_without_PI
  ...
  Components.BaseComponents.Air.Volumes.AirVolume airVolume(Tstart = 273.15 + 25, V = Vroom) annotation(
    Placement(transformation(extent = {{-6, -66}, {44, -26}})));
  ...
  Components.BaseComponents.HVAC.HeatPumps.HeatPump_ConstantCOPcool heatPump_ConstantCOPcool(Wmax = 1500, constCOPcool = 3) annotation(
    Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {20, 48})));
  Modelica.Blocks.Interfaces.RealInput Command_Heater annotation(
...
equation
  connect(airVolume.air_flange2, airSink_P_fixed.air_flange) annotation(
    Line(points = {{39, -46}, {48, -46}}, color = {170, 255, 255}, smooth = Smooth.None));
  ...
  connect(convSca2Sca5.ss2, airVolume.heatPort) annotation(
    Line(points = {{66, 1.11022e-015}, {64, 1.11022e-015}, {64, 14}, {19, 14}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  connect(convSca2Sca1.ss1, airVolume.heatPort) annotation(
    Line(points = {{-1.66533e-015, 74}, {-1.66533e-015, 68}, {64, 68}, {64, 14}, {19, 14}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  connect(convSca2Sca6.ss1, airVolume.heatPort) annotation(
    Line(points = {{-60, 2}, {-58, 2}, {-58, 68}, {64, 68}, {64, 14}, {19, 14}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  connect(convSca2Sca3.ss2, airVolume.heatPort) annotation(
    Line(points = {{-2.22045e-016, -68}, {-2.22045e-016, -66}, {64, -66}, {64, 14}, {19, 14}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  ...
  connect(prescribedHeatFlow.port, airVolume.heatPort) annotation(
    Line(points = {{26, -6}, {19, -6}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  ...
  connect(temperatureSensor4.port, airVolume.heatPort) annotation(
    Line(points = {{6, -16}, {19, -16}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  ...
  connect(convSca2Sca4.ss1, airVolume.heatPort) annotation(
    Line(points = {{-33.6, -34}, {-22, -34}, {-22, -30}, {19, -30}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  connect(convSca2Sca2.ss1, airVolume.heatPort) annotation(
    Line(points = {{-33.6, -56}, {-22, -56}, {-22, -30}, {19, -30}, {19, -28}}, color = {255, 128, 0}, smooth = Smooth.None));
  ...
  connect(heatPump_ConstantCOPcool.coldPort, airVolume.heatPort) annotation(
    Line(points = {{29, 48}, {38, 48}, {38, 14}, {19, 14}, {19, -28}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(gain.u, Command_Heater) annotation(
    Line(points = {{3, 23}, {-52, 23}, {-52, 32}, {-108, 32}}, color = {0, 0, 127}, smooth = Smooth.None));
  annotation(
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-120, -120}, {120, 120}}), graphics),
    Icon(coordinateSystem(extent = {{-120, -120}, {120, 120}}, preserveAspectRatio = false), graphics = {Rectangle(extent = {{-60, 60}, {60, -60}}, lineColor = {0, 0, 0}, fillColor = {170, 255, 213}, fillPattern = FillPattern.Solid), Text(extent = {{-36, 22}, {36, -20}}, lineColor = {0, 0, 0}, fillColor = {0, 255, 0}, fillPattern = FillPattern.Solid, textString = "%name
    ")}));
end Room_without_PI;